...
dojo.declare("Spring.ValidateAllDecoration", [Spring.AbstractValidateAllDecoration, Spring.DefaultEquals], {
	...
	apply : function() {
		...
		//sstagni - uniformata la riga successiva con quella delle altre decorazioni (altrimenti il validate non funzionerebbe sui widget)		
...
dojo.declare("Spring.AjaxEventDecoration", [Spring.AbstractAjaxEventDecoration, Spring.DefaultEquals], {
	constructor : function(config){
		...
		this.connection = null;
		this.allowed = true;
		this.applyConfirm = ''; //sstagni - aggiunto per confirm in caso di ajax
		dojo.mixin(this, config);
	},
	...
	submit : function(event){
		...
		//sstagni
		if (this.applyConfirm != '') {
			if (!confirm(this.applyConfirm)){
				dojo.stopEvent(event);
				return false;
			}			
		}
